...
class WP_CustomMenu_Block_Navigation extends Mage_Catalog_Block_Navigation
{
    ...
    public function drawCustomMenuMobileItem($category, $level = 0, $last = false)
    {
        ...
        if($category->getData('category_label')){
            $html[] = '<span class="label"><span class="icon"></span>' . $category->getData('category_label') . '</span>';
        }
        $html[] = '</a>';
        if ($hasSubMenu) {
    ...
    public function drawMobileMenuItem($children, $level = 1)
    {
        ...
        foreach ($children as $child) {
            if (is_object($child) && $child->getIsActive()) {
                ...
                $html.= '<span class="label"><span class="icon"></span>' . $child->getData('category_label') . '</span>';
                }
                $html.= '</a>';
    ...
    public function drawMenuItem($children, $level = 1)
    {
        ...
        foreach ($children as $child)
        {
            if (is_object($child) && $child->getIsActive())
            {
                if($ct == 0 && $level == 1 && Mage::helper("ExtraConfig")->themeOptions('categoryimage') == 1 && $this->_getThumbnailImageUrl($child->getId()) != false)
                {   
                    $html .= '<div class="category-image"><img src="'.$this->_getThumbnailImageUrl($child->getId()).'" /></div>';
                    $ct=1;
                }
